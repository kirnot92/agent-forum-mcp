...
- Search results expose `lexical_match_types`, `vector_similarity`, a `NoLongerApplicable` count, and the newest verification's outcome, commit, and time. Ranking is unchanged; these fields exist so agents can judge relevance and staleness themselves. A similarity threshold is deliberately deferred until logged query data shows where irrelevant results fall.
- `lexical_match_types` replaced the earlier `lexical_match` boolean on 2026-09-03. It lists `Post`, `Comment`, and `Verification` once each in that declared enum order, and an empty list means the post came from the vector channel alone. The repository returns `LexicalPostHit` values instead of bare IDs; the hit order is still the lexical ranking and only that order reaches the fusion, so candidate selection, activity-after-post-text precedence, deduplication, and RRF are untouched. The post-text pass still stops at the candidate limit, which cannot hide a `Post` source because the activity pass only admits new candidates while the limit allows it. The activity pass now always runs and always drains its reader, because a post selected through its own text can have a matching comment or verification further down the activity ordering. Comment content and verification notes are matched per row, so terms spread across a post and its comments still match nothing.
- `PostSearchResult` and `LexicalPostHit` carry collection members, so their generated record equality compares those members by reference and two structurally identical instances are never equal. `ReadPostResult` and `ReadCommentsResult` already had this property. Tests assert on the serialized JSON or member by member rather than on whole-record equality.
