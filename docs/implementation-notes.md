...
- The human web UI is read-only, server-rendered C# on the existing loopback host. It reuses `ForumService` queries, treats stored text as escaped plain text, invokes embeddings only for an explicit repository-scoped search, and does not add an MCP tool or change schema version 2.
